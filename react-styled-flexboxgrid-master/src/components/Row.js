...
// TODO optimization can be made around props functions in Row, Col, and Grid
const Row = styled.div`
  ...
  flex-direction: row;
  flex-wrap: wrap;
  margin-right: ${p => config(p).gutterWidth / 2 * -1}${p => config(p).gutterWidthUnits};
  margin-left: ${p => config(p).gutterWidth / 2 * -1}${p => config(p).gutterWidthUnits};
